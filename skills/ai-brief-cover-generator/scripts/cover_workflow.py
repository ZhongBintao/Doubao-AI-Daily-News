...
POSITIVE_REFERENCE_PATH = SKILL_DIR / "assets" / "references" / "cover-positive-16x9.png"
...
def build_prompt_seedream(
    *,
    ...
    """Build a Seedream-friendly Chinese prompt.

    The original GPT Image prompt uses English structural labels
    (``Use case:``, ``Constraints:``, etc.) that GPT Image recognises as
    instructions. Seedream renders those labels as on-canvas text instead.
    This variant keeps every piece of content from the original prompt but
    rewrites it as flowing Chinese natural language with a guard clause, so
    no structural label appears in the generated image.
    """
    if generation_role not in {"anchor", "adaptation"}:
        raise CoverWorkflowError(f"unsupported generation role: {generation_role}")
    if generation_role == "adaptation" and not anchor_ratio:
    ...
    brand_names = "、".join(value["name"] for value in brands) or "本期无可核实品牌"
    adaptation_note = ""
    if generation_role == "adaptation":
        adaptation_note = (
            f"另外参考本期 {anchor_ratio} 封面，只继承其视觉概念和系列气质，"
            f"然后为 {ratio.ratio} 竖版完全重新构图；不要裁切、拉伸或填充。"
        )

    return f"""以下是封面创作指令，不要把指令本身的任何文字画到画面上。
请参考输入图片的视觉风格和版面布局，生成一张全新的 AI 每日早报完整平面封面，比例 {ratio.ratio}，用于新闻类内容发布。学习参考图的暖象牙色底色、橙黑配色、3D 等距信息图插画质感、圆角品牌胶囊和柔和阴影，但不要复制参考图中的任何具体文字、日期、数字、插画元素或品牌。{adaptation_note}

画面布局：左上角是栏目名和日期，左侧是大标题和副标题胶囊，右侧是与新闻主题相关的 3D 等距风格信息图插画，底部或右侧是品牌标识胶囊。

封面上必须准确呈现以下四段文字，每段只出现一次：
"AI每日早报"
"{date_label}"
"{headline}"
"{subheadline}"

本期新闻视觉主题：{visual_brief.strip()}

本期品牌：{brand_names}。品牌以白色圆角胶囊形式出现在画面中，胶囊内是品牌名称或 Logo，不规定固定位置，按比例重新组织。

整体风格：暖象牙色明亮底色；黑色超粗标题字；橙红色重点词和圆角信息条；深青色与橙色的编辑型信息图插画；轻微立体材质和柔和阴影；栏目名和日期形成次级层级；主标题具有压倒性视觉权重；新闻隐喻、趋势箭头、数据路径、设备或模型符号与品牌区域形成一个完整叙事。

除了上面指定的四段文字外，不要出现任何其他文字、数字、伪文字、水印、二维码、播放器界面、手机边框或签名。生成完整成品，不留待排版空白。"""
...
def prepare_request(args: argparse.Namespace) -> dict[str, Any]:
    ...
    image_provider = str(getattr(args, "image_provider", "gpt-image") or "gpt-image").strip().lower()
    positive_reference = None
    if image_provider == "seedream" and POSITIVE_REFERENCE_PATH.is_file():
        positive_reference = _reference_record(
            POSITIVE_REFERENCE_PATH, "seedream_positive_layout_reference"
        )
        positive_reference["content_policy"] = "learn_layout_and_visual_quality_only_do_not_copy_specific_elements"
    ...
    for spec in specs:
        ...
        if positive_reference is not None:
            references.append(dict(positive_reference))
        if role == "adaptation":
            references.append({
        ...
        value.update({
            ...
            "prompt": gpt_prompt,
            "seedream_prompt": build_prompt_seedream(
                date_label=date_label,
                headline=args.headline.strip(),
                subheadline=args.subheadline.strip(),
                ratio=spec,
                ...
                generation_role=role,
                anchor_ratio=anchor_ratio,
            ) if image_provider == "seedream" else None,
        })
    ...
    request = {
        ...
        "generation_policy": {
            "strategy": "first_result_direct" if image_provider != "seedream" else "first_result_with_text_accuracy_retry",
            "one_request_per_ratio": True,
            "attempts_per_ratio": 1 if image_provider != "seedream" else 2,
            "retries": 0 if image_provider != "seedream" else 1,
            "human_confirmation": False,
            "image_review": False,
            "programmatic_visual_validation": False,
            "text_accuracy_check": image_provider == "seedream",
        },
        "selected_item": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare_parser.add_argument("--image-provider", default="gpt-image", choices=["seedream", "gpt-image"], help="image generation provider; seedream adds a Chinese prompt and positive-layout reference (default: gpt-image)")
